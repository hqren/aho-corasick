...
public class TestAhoCorasick extends TestCase {
	...
	tree.add("hello".getBytes(), 1);
	tree.add("hi".getBytes(), 2);
	tree.prepare();
	...
	tree.add("he".getBytes(), 1);
	tree.add("she".getBytes(), 2);
	tree.add("his".getBytes(), 3);
	tree.add("hers".getBytes(), 4);
	...
	tree.add("john".getBytes(), 1);
	tree.add("jane".getBytes(), 2);
	tree.prepare();
	...
	tree.add("x".getBytes(), 1);
	tree.add("xx".getBytes(), 2);
	tree.add("xxx".getBytes(), 3);
	tree.prepare();
	...
	tree.add("moo".getBytes(), 1);
	tree.add("one".getBytes(), 2);
	tree.add("on".getBytes(), 3);
	tree.add("ne".getBytes(), 4);
	tree.prepare();
	...
	for (int i = 0; i < terms.length; i++) {
	    tree.add(terms[i].getBytes(), i);
	}
	tree.prepare();

	TIntHashSet termsThatHit = new TIntHashSet();
	for (Iterator iter = tree.search(text.getBytes()); iter.hasNext(); ) {
	    SearchResult result = (SearchResult) iter.next();
      TIntIterator iterator = result.getOutputs().iterator();

      while(iterator.hasNext()){
        termsThatHit.add(iterator.next());
      }
	}
	assertEquals
